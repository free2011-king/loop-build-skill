...
class CreateLoopPackTests(unittest.TestCase):
    def test_scaffold_contains_role_boundaries_and_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            product_workspace = (
                loop_dir / "roles" / "product-workflow" / "workspace.md"
            ).read_text(encoding="utf-8")
            ...
            self.assertTrue((loop_dir / "roles" / "product-workflow" / "workspace.md").exists())
            self.assertTrue((loop_dir / "roles" / "project-management" / "workspace.md").exists())
            self.assertTrue((loop_dir / "roles" / "development" / "workspace.md").exists())
            self.assertTrue((loop_dir / "roles" / "testing-evaluation" / "workspace.md").exists())
            ...
            self.assertIn("Product Manager Capability Baseline", product_workspace)
            self.assertIn("user needs before turning demand into build scope", product_workspace)
            self.assertIn("outcome metric", product_workspace)
            self.assertIn("evidence links or gaps", product_workspace)
