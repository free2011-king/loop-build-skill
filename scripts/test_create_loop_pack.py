...
class CreateLoopPackTests(unittest.TestCase):
    def test_scaffold_contains_role_boundaries_and_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            role_sessions = (loop_dir / "role-sessions.md").read_text(encoding="utf-8")
            ...
            self.assertIn("Initial real Codex thread cap", project_metadata)
            self.assertIn("3 or fewer", project_metadata)
            self.assertIn("shared implementation thread", project_metadata)
            self.assertIn("Thread Split Triggers", project_metadata)
            ...
            self.assertIn("Initial real Codex threads: 3 or fewer", team_formation)
            self.assertIn("Multiple roles sharing one thread", team_formation)
            ...
            self.assertIn("real Codex threads capped at 3 or fewer", candidate_roles)
            self.assertIn("shared-thread role grouping", candidate_roles)
            self.assertIn("Dedicated thread creation is a split decision", candidate_roles)
            ...
            self.assertIn("Initial Thread Budget", role_sessions)
            self.assertIn("3 or fewer real threads", role_sessions)
            self.assertIn("shared discovery/implementation thread", role_sessions)
            self.assertIn("Acting role marker", role_sessions)
            self.assertIn("Real Thread Count After Change", role_sessions)
