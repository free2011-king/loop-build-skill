...
class CreateLoopPackTests(unittest.TestCase):
    def test_scaffold_contains_role_boundaries_and_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project_metadata = (loop_dir / "project-metadata.md").read_text(encoding="utf-8")
            ...
            role_loop_charters = (loop_dir / "role-loop-charters.md").read_text(
                encoding="utf-8"
            )
            ...
            stage_confirmation = (loop_dir / "stage-confirmation-checklist.md").read_text(
                encoding="utf-8"
            )
            ...
            self.assertIn("project-metadata.md", document_index)
            ...
            self.assertIn("project-metadata.md", loop_design)
            self.assertTrue((loop_dir / "project-metadata.md").exists())
            self.assertIn("Project Metadata", project_metadata)
            self.assertIn("Role And Division Metadata", project_metadata)
            self.assertIn("Thread / Session Metadata", project_metadata)
            self.assertIn("Metadata Change Log", project_metadata)
            self.assertIn("Loop Manager owns this file", project_metadata)
            self.assertIn("role-registry.md", project_metadata)
            self.assertIn("role-sessions.md", project_metadata)
            self.assertIn("LM-001", project_metadata)
            ...
            self.assertIn("Product Thinking And Clarification Loop", candidate_roles)
            self.assertIn("not a task transcriber", candidate_roles)
            self.assertIn("Demand is ready for handoff only when", candidate_roles)
            ...
            self.assertIn("Default Automatic Distillation Time Nodes", loop_manager)
            self.assertIn("Project-start baseline", loop_manager)
            self.assertIn("Task or stage completion checkpoint", loop_manager)
            self.assertIn("Project-closure distillation", loop_manager)
            ...
            self.assertIn("Project Owner Core Operating Principles", loop_manager)
            self.assertIn("Acceptance Translation Packet", loop_manager)
            self.assertIn("Verifiable Acceptance Standards", loop_manager)
            self.assertIn("Project metadata review", loop_manager)
            self.assertIn("project-metadata.md", loop_manager)
            ...
            self.assertIn("Role skill-evolution checkpoint", loop_manager)
            self.assertIn("Role Skill Evolution Backlog", loop_manager)
            self.assertIn("Proposed Skill / Checklist / Prompt / Tool Update", loop_manager)
            self.assertIn("Skill Evolution Notes", demand_workspace)
            self.assertIn("Skill Evolution Notes", loop_manager_workspace)
            self.assertIn("Role Skill Evolution Pattern", candidate_roles)
            self.assertIn("observed skill signal -> proposed update", candidate_roles)
            self.assertIn("Project Owner Acceptance Translation Pattern", candidate_roles)
            self.assertIn("Core operating principles", candidate_roles)
            self.assertIn("private interpretation", candidate_roles)
            self.assertIn("private understanding as execution permission", candidate_roles)
            self.assertIn("Hard Constraint Gate Pattern", candidate_roles)
            self.assertIn("Product/UI/workflow change confirmation", candidate_roles)
            self.assertIn("Developer refuses to implement against unconfirmed", candidate_roles)
            self.assertIn("Standardized production with clear requirements", candidate_roles)
            self.assertIn("automated-pass", candidate_roles)
            self.assertIn("Hard Constraint Gates", loop_manager)
            self.assertIn("HCG-001", loop_manager)
            self.assertIn("HCG-002", loop_manager)
            self.assertIn("Product/UI/workflow changes need user/domain-owner confirmation", loop_manager)
            self.assertIn("human-required / delegated-review / automated-pass", loop_manager)
            self.assertIn("Hard constraint gate rule", implementation_plan)
            self.assertIn("Human participation rule", implementation_plan)
            self.assertIn("Hard Constraint And Human Participation Confirmation", stage_confirmation)
            self.assertIn("Acceptance Translation Confirmation", stage_confirmation)
            self.assertIn("Verifiable Acceptance Standards", stage_confirmation)
            self.assertIn("Human Participation", stage_confirmation)
            self.assertIn("Downstream Roles Blocked Until Confirmed", stage_confirmation)
            ...
            self.assertIn("Hard Constraint Gates", implementation_covenant)
            self.assertIn("Gate Enforcement By Role", implementation_covenant)
            self.assertIn("Refuse implementation against unconfirmed", implementation_covenant)
            self.assertIn("standardized execution roles", implementation_covenant)
            self.assertIn("requirements are clear, authority is delegated", implementation_covenant)
            ...
            self.assertIn("Acceptance Translation Before Dispatch", goal_dispatch_log)
            self.assertIn("Verifiable Acceptance Standards / Confirmation", goal_dispatch_log)
            ...
            self.assertIn("Acceptance Standards Before Execution", loop_manager_workspace)
            self.assertIn("Hard Constraint Gate Responsibility", loop_manager_workspace)
            self.assertIn("Blocked Action If Missing", loop_manager_workspace)
            self.assertIn("Standardized production with clear requirements", loop_manager_workspace)
            self.assertIn("Project Owner Status Sync", loop_manager_workspace)
            self.assertIn("Translate user demand into verifiable acceptance standards", role_loop_charters)
            self.assertTrue((loop_dir / "role-registry.md").exists())
            self.assertIn("Active Role Registry", role_registry)
            self.assertIn("project-metadata.md", role_registry)
