...
- Apply `如无必要，勿增实体`: add roles, stages, tools, records, gates, threads, or abstractions only when they close the Loop, reduce meaningful risk, preserve reusable learning, or satisfy a confirmed constraint. In early project stages, keep the active role set small: Loop Manager is mandatory, and the other active roles should usually start with 2-3 roles selected for the next Loop round. Keep real Codex threads even smaller at initialization: do not exceed 3 real threads unless the user confirms or Loop Manager records evidence that separation is needed. Multiple roles may share one thread when their boundaries, current speaker, records, and handoffs are explicit. Separate role files or workspaces are allowed for clarity; avoid activating too many specialized roles or threads before confirmed complexity, workload, risk, or workflow evidence requires splitting them out.
...
- Require the project owner role to translate user demand into verifiable acceptance standards before execution. Whether the role is a project manager, Loop Manager acting temporarily, or Loop Owner, it must not treat "I understand the user" as permission to execute. It must produce clear acceptance criteria, confirmation questions, evidence expectations, and a user/domain-owner confirmation record first.
...
- Classify human participation deliberately. Require human participation for aesthetic judgment, taste, brand feel, product direction, user value tradeoffs, ambiguous demand, governance/risk authorization, or changes to confirmed constraints. Do not require user participation for standardized production when requirements are clear, authority is delegated, evidence is available, deterministic checks pass, and no confirmed constraint changes.
...
- Initialize every concrete Loop project with `project-metadata.md` for project identity, role information, division of work, thread/session basics, and metadata change history. Loop Manager maintains it as roles, responsibilities, project owner, workspaces, and sessions change.
...
3. Confirm verifiable acceptance before execution.
   The project owner role translates user language into acceptance standards with observable pass/fail checks, examples, non-goals, evidence requirements, and confirmation owner. Ask the user/domain owner to confirm these standards before implementation or standardized production begins.

4. Create Loop governance first.
...
6. Select implementation roles conservatively.
   Match confirmed needs to existing candidate roles first, but start with the smallest active role set that can responsibly run the next Loop round. A normal early project shape is Loop Manager plus 2-3 other active roles; choose those roles by project nature instead of by habit. Keep the initial real Codex thread plan at 3 or fewer threads by grouping multiple roles into shared role threads when safe. Prepare selected roles, deferred candidate roles, excluded roles, split triggers, reasons, role categories, workspaces, shared-thread assignments, authority boundaries, tool/skill readiness, and first handoff path for user/domain-owner confirmation before implementation. Do not activate specialized roles merely because they exist in the candidate library.

7. Track reusable role capability sources.
...
8. Evolve role skills gradually.
...
9. Establish role operating rules.
...
10. Define hard constraint gates.
...
11. Classify human participation points.
   Mark each gate or stage as `human-required`, `delegated-review`, or `automated-pass`. Human-required points are usually aesthetic, product, value, risk, or authorization decisions. Automated-pass points are standardized production steps with clear requirements, delegated authority, traceable evidence, and objective checks.

12. Design the smallest full cycle.
...
13. Choose observation and update mechanisms.
...
14. Scaffold and customize artifacts when useful.
...
15. Stop at gates when confirmation is missing.
...
- If no project manager is selected, the project owner duty may be temporarily carried by Loop Manager or Loop Owner, but only to translate demand into verifiable acceptance standards, obtain confirmation, and dispatch; it is not permission to execute role-specific work.
...
- Role/personnel/session changes are not complete until Loop Manager updates `project-metadata.md` and `role-registry.md`, notifies every other active role, and records the broadcast.
- At project initialization, real Codex threads should not exceed 3. Use shared threads for multiple roles until workload, context separation, independent review, confidentiality, risk isolation, or repeated handoff friction justifies a dedicated thread. Role count, role files, and role workspaces may exceed thread count when metadata and handoff records keep boundaries clear.
...
- Human participation is not the default for every standardized step. Put humans where judgment is needed; let clear, standard, low-risk, delegated production proceed through objective evidence and recorded checks.

## Output Shape

For a Loop-design answer, provide the Loop owner, Loop Manager, project metadata path/status, project owner / project-manager delegation, verifiable acceptance standards and confirmation status, initial real thread plan capped at 3 threads, shared-thread role grouping, thread split triggers, Loop Manager automatic distillation time nodes, role skill-evolution mechanism, hard constraint gates, human participation classification, initial minimal active role set, deferred candidate roles and split triggers, role categories, Loop object, minimum viable loop, key records, feedback/evaluation signals, update mechanism, first implementation order, selected/excluded roles, implementation covenant status, category workspaces, interaction evidence rule, disagreement/escalation path, risks, confirmation needed, and artifact path if files were created.
